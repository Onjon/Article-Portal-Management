		...
		function checkarticle() {			
			var a , b , c , f1 , f2 , f3 , arrayOfFiles , len , sz , i , j , xmlhttp , fileReader , imgData , tempFileName ;
		...
		function articleOnkeyUp() {
			...
			res = parseInt( res ) ;
			if( res <= 80 && wordCount( localData ) >= 10 && wordCount( localData ) <= 200 ) {
				return true ;
			}
		...
		function titleOnkeyUp() {
			...
			res = parseInt( res ) ;
            console.log( res ) ;
			if( res <= 50 && wordCount( localData ) >= 5 && wordCount( localData ) <= 20 ) {
				return true ;
			}
