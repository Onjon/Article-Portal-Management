		...
		*/
		$content.find('form').wl_Form({
			onSuccess: function(data, status){
				...
				var formData = {fname:$('#fname').val(),lname:$('#lname').val(),email:$('#email').val(),amount:$('#amount').val()};
				...
				else
				{
					...
					url: "parse/adduser.php",
					type: "POST",
					data: formData,
					success: function(data, textStatus, jqXHR)
					{
						$content.find('form .reset').click();
						$.msg("User created and an email hes been sent to the user's email.");
					},
		...
		$('.adminform').click(function(){
			...
			else
			{
				$.ajax({
					...
					$.msg("Admin created successfully and an email has been sent.");
			}
		});
		
		//Change Password
		$('.changepass').click(function(){
			//var formData = {cpass:$('#cpass').val(),npass:$('#npass').val(),cnpass:$('#cnpass').val()};
		...
		$('.delete').click(function(){
			var a = $(this).parent('td').prev('td').text();
			location.href="parse/delete.php?userid="+a;
			$.msg("User Data Deleted");
			
		});
		
		$('.disable').click(function(){
			var a = $(this).parent('td').parent('tr').children('td.user').text();
			location.href="parse/disable.php?userid="+a;
		...
		$('.enable').click(function(){
			var a = $(this).parent('td').parent('tr').children('td.user').text();
			location.href="parse/enable.php?userid="+a;
